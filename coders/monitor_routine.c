...
void    send_broadcast_state_dongle(t_data *data)
{
    ...
    pthread_broadcast(&data->cond_state_dongles, &data->mutex_state_dongles);
...
void    check_all(t_data *data)
{
    ...
    while (i < data->n_coders)
    {
        ...
        if (get_compile_done(&data->coders[i]) < data->n_compile)
            all_done = 0;
        if (get get_last_comp_start(&data->coders[i]) + data->t_burnout < get_timestamp(data))
